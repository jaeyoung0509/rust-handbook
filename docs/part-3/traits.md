...
`Display`는 사람에게 보여줄 메시지, `Error::source()`는 cause chain, `From`은 lower-level failure를 higher-level contract로 올리는 bridge다. 이 세 가지를 같이 보면 custom error가 단순한 enum이 아니라 API surface라는 점이 보인다.
